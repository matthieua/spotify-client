...
module Spotify
  class Client
    ...
    def remove_tracks(tracks_ids)
      run(:delete, '/v1/me/tracks', [200], JSON.dump(tracks_ids))
    end

    def remove_albums(albums_ids)
      params = { ids: Array.wrap(albums_ids).join(',') }
      run(:delete, '/v1/me/albums', [200], params)
    end
    ...
    def user_playlists(user_id, limit = 20, offset = 0)
      run(:get, "/v1/users/#{user_id}/playlists?limit=#{limit}&offset=#{offset}", [200])
    end
